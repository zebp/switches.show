var manifest = {
	...
	{
		type: "script",
		href: "/assets/index-f18ee725.js"
	},
	{
		type: "script",
		href: "/assets/entry-client-6f57fea0.js"
	},
	{
		type: "style",
		href: "/assets/entry-client-1de85277.css"
	}
],
	"entry-client": [
	{
...
function getAssetsFromManifest(manifest, routerContext) {
  ...
  const links = match.reduce((r, src) => {
    r[src.href] = src.type === "style" ? ssr(_tmpl$$2, ssrHydrationKey(), ssrAttribute("href", escape(src.href, true), false)) : src.type === "script" ? ssr(_tmpl$2$1, ssrHydrationKey(), ssrAttribute("href", escape(src.href, true), false)) : undefined;
    return r;
  }, {});
...
const _tmpl$$1 = ["<div", " class=\"", "\" style=\"", "\"><span>", "</span></div>"];
function Key({
  label,
  width,
  pressed = false
}) {
  const colors = pressed ? "bg-neutral-800 border-neutral-700" : "bg-neutral-900 border-neutral-800";
  return ssr(_tmpl$$1, ssrHydrationKey(), `text-white h-14 border-2 rounded-md border-b-4 flex items-center justify-center ${escape(colors, true)}`, "min-width:" + `calc(3.5rem * ${escape(width, true)})` + (";max-width:" + `calc(3.5rem * ${escape(width, true)})`), escape(label));
}

const _tmpl$ = ["<main", " class=\"flex items-center justify-center p-16 bg-black w-screen h-screen\"><div class=\"flex flex-col gap-1\">", "</div></main>"],
  _tmpl$2 = ["<div", " class=\"flex gap-1\">", "</div>"];
const QUERY = [["`", "1", "2", "3", "4", "5", "6", "7", "8", "9", "0", "-", "=", ["Backspace", 2]], [["Tab", 1.5], "Q", "W", "E", "R", "T", "Y", "U", "I", "O", "P", "[", "]", ["\\", 1.5]], [["Caps", 1.75], "A", "S", "D", "F", "G", "H", "J", "K", "L", ";", "'", ["Enter", 2.25]], [["Shift", 2.25], "Z", "X", "C", "V", "B", "N", "M", ",", ".", "/", ["Shift", 2.75]], [["Ctrl", 1.25], ["Win", 1.25], ["Alt", 1.25], ["", 7.5], "Alt", ["Win", 1.25], ["Menu", 1.25], ["Ctrl", 1.25]]];
function createKey(item) {
  if (Array.isArray(item)) {
    return createComponent(Key, {
      ...
      get pressed() {
        return ["Win", "Enter"].includes(item[0]);
      }
    });
  }
  return createComponent(Key, {
    ...
    width: 1,
    get pressed() {
      return ["S", "B", "P", "7"].includes(item);
    }
  });
}
function Home() {
  return ssr(_tmpl$, ssrHydrationKey(), escape(QUERY.map(row => ssr(_tmpl$2, ssrHydrationKey(), escape(row.map(label => createKey(label)))))));
}
